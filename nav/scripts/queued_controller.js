const cookiesEnabled = navigator.cookieEnabled;
    ...
    var uuid = getCookie('sessionUuid82');

    function initSessionConn() {
    	if (sessionConn === null) {
    		const uuidExtension = uuid ? `?uuid=${uuid}` : '';

    		sessionConn = new WebSocket("wss\u003A\/\/oberlin.communityhub.cloud/digital-signage/websockets/session/82" + uuidExtension);
    ...
    function sessionConnReceiver(e) {
    	...
    	if (command.type === 'restart') {
    		...
    		uuid = command.uuid;
    		setCookie('sessionUuid82', command.uuid, 3650);
